...
from final_assesment.data_util.get_train_test_data import get_train_test_data, get_test_data_fin
from final_assesment.randomforest.rf_grid_search import classification_report_with_accuracy_score

n_jobs = 40
rf1 = RandomForestClassifier(max_depth=40, n_estimators=20, n_jobs=n_jobs, oob_score=False)
rf2 = RandomForestClassifier(max_depth=20, n_estimators=60, n_jobs=n_jobs, oob_score=False)
rf3 = RandomForestClassifier(max_depth=30, n_estimators=150, n_jobs=n_jobs, oob_score=False)


clfSoft = VotingClassifier(estimators=[('rf1', rf1), ('rf2', rf2), ('rf3', rf3)], voting='hard')

X_train, y_train, X_val, y_val = get_train_test_data()
X_test, y_test = get_test_data_fin()
